// Performance load test - converted to Jest format
    ...
    for (let i = 0; i < 10; i++) {
      promises.push(
        axios.get(`${baseURL}/api/health`)
          .then(response => ({ status: response.status, time: performance.now() - startTime }))
          .catch(error => ({ error: error.message, time: performance.now() - startTime }))
      );
    }
