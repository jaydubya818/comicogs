...
class PricingData {
  static async createTables() {
    ...
    try {
      // Drop tables if they exist to ensure a clean state for testing
      await db.query('DROP TABLE IF EXISTS pricing_data CASCADE;');
      await db.query('DROP TABLE IF EXISTS price_history CASCADE;');
      await db.query('DROP TABLE IF EXISTS marketplaces CASCADE;');
      await db.query('DROP TABLE IF EXISTS collection_jobs CASCADE;');
  ...
  static async getCurrentPricing(comicId, options = {}) {
    const { marketplace = null, condition = null, grade = null, limit = 50, days = 30 } = options;
  ...
  static async getPriceTrends(comicId, options = {}) {
    const { marketplace = null, condition = null, days = 180 } = options;
  ...
  static async insertPriceHistory(data) {
    const query = `
    ...
    try {
      await db.query(query, [data.comic_id, data.marketplace, data.date_period]);
      return true;
    } catch (error) {
