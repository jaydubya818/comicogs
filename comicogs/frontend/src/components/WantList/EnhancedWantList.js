...
const WantListItem = ({ item, onNotify, onDelete }) => (
    <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-white rounded-lg shadow-sm p-4 flex items-start space-x-4">
        <img src={item.imageUrl} alt={item.title} className="w-16 h-24 object-cover rounded-md" />
        <div className="flex-grow">
            <h3 className="font-bold text-gray-800">{item.title} #{item.issue}</h3>
            <div className="flex items-center gap-2 mt-1">
                <PriorityBadge priority={item.priority} />
                <span className="text-sm text-gray-500">Max Price: ${item.maxPrice.toFixed(2)}</span>
            </div>
            {item.notes && <p className="text-sm text-gray-600 mt-2 italic">Notes: {item.notes}</p>}
        </div>
        <div className="flex-shrink-0 flex flex-col items-end gap-2">
            <button onClick={() => onNotify(item.id)} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-full"><Bell size={18} /></button>
            <button onClick={() => onDelete(item.id)} className="p-2 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-full"><Trash2 size={18} /></button>
        </div>
    </motion.div>
);

const MatchItem = ({ match }) => (
    <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center justify-between">
        <div>
            <h4 className="font-semibold text-green-800">Match found for {match.title}</h4>
            <p className="text-sm text-green-700">Price: ${match.price.toFixed(2)} from {match.seller}</p>
        </div>
        <a href={match.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700">
            ...
            <div className="max-w-4xl mx-auto">
                <header className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-gray-600">Track your most desired comics and get notified of matches.</p>
                    </div>
                    <button onClick={handleAdd} className="flex items-center gap-2 px-4 py-2 font-medium text-white bg-blue-600 rounded-lg shadow-sm hover:bg-blue-700">
                        <Plus size={18} />
                    ...
                    <AnimatePresence>
                        {wantList.length > 0 ? (
                            ...
                            <div className="text-center py-12 bg-white rounded-lg shadow-sm">
                                ...
                                <h3 className="mt-4 text-lg font-semibold text-gray-800">Your want list is empty</h3>
                                <p className="mt-1 text-gray-500">Click "Add to List" to start tracking comics.</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
